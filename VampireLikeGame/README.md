...
A Vampire Survivors–style auto-battler prototype for Unity, themed as a 東方Project (Touhou) fan game:
move Reimu around a shrine arena while spell cards auto-attack the nearest fairies, collect power items,
and pick a spell/stat upgrade every time you level up.

## About the Touhou theming

Only names/terms are used here (陰陽玉, 結界, 妖精/大妖精 as the generic mob types, etc.) — no official
character art, sprites, music, or voice data are included or referenced anywhere; all visuals are still
the flat-colored placeholder shapes generated by the bootstrapper. ZUN's derivative-work guidelines are
broadly permissive toward non-commercial Touhou fan games, but they still don't cover redistributing
official assets — if you add real game sprites/music later, that's on you to source/license properly,
and a wide commercial release is a separate conversation with those guidelines.
